...
void ePump::PrepareAM(int k) {
    ...
    for (int i=0;i<Ni;i++) {
        ...
        for (int a=0;a<Na[k];a++) {
            ...
            for (int b=0;b<Na[k];b++) {
                ...
                temp2[i][a]+=Cm[k][a][b]*XXdyn[k][b][i];
            }
            Atemp[i]+=(XE[k][a]-X0[k][a])*temp[i][a];
            Btemp[i]+=(XE[k][a]-X0[k][a])*temp2[i][a];
        }
        Atemp[i]*=(weight[k]/Tsq);
        Btemp[i]*=(weight[k]/Tsq);
        A[i]+=Atemp[i];
    }
    ...
    for (int i=0;i<Ni;i++) {
        for (int j=i;j<Ni;j++) {
            ...
            IplusM[i][j]+=Mtemp[i][j];
            if (i!=j) IplusM[j][i]=IplusM[i][j];
        }
        if (M_flag==1) {
            IplusM[i][i]-=Btemp[i];
        }
...
//    d_vec dYT(Ni);
    ...
    for (int k=0;k<Ndata;k++) {
        for (int a=0;a<Na[k];a++) {
//            for (int i=0;i<Ni;i++) {
//                dYT[i]=dX[k][a][i]/TdynNorm[i];
//            }
            BackSub(L,Xdyn[k][a],LinvXdyn,Ni);
            X0new[k][a]=X0[k][a]+dot(LinvXdyn,LinvA,Ni);
        }
    }
    ...
    for (int i=0;i<Ni;i++) {
        ...
        double dd=dot(LinvIdent[i],LinvA,Ni);
        //
        // dd = z^0_i     !!!!!!!!!
        //
        Chi20new+=dd*dd;
...
void ePump::UpdateErrors0(const d_vec &ddY, double &DY0, double &DYnew,double &Yshift)
                 // Give error vector dY as input to calculate Yshift and DY0, DYnew.
{
    d_vec LinvdY(Ni);
    d_vec LinvdYT(Ni);
    d_vec ddYT(Ni);
    for (int i=0;i<Ni;i++) {
        ddYT[i]=ddY[i]/TdynNorm[i];
    }
    BackSub(L,ddY,LinvdY,Ni);
    BackSub(L,ddYT,LinvdYT,Ni);
    Yshift=dot(LinvdYT,LinvA,Ni);
    DY0=sqrt(dot(ddY,ddY,Ni));
    DYnew=sqrt(dot(LinvdY,LinvdY,Ni));
}

void ePump::UpdateErrors(const d_vec &YY, double &DY0sym, double &DY0up, double &DY0down,
                   double &DYnewSym, double &DYnewUp, double &DYnewDown, double &Yshift)
                 // Give 2*Ni+1 dimensional vector of Y values as input, with
                 //       YY[0]=Y[0] (central value), YY[2*i+1]=Y[i+], YY[2*i+2]=Y[i-].
                 // Calculates Yshift, and both symmetric and asymmetric versions of the errors:
                 //      DY0sym, DY0up, DY0down,band DYnewSym, DYnewUp, DYnewDown.
{
    ...
    UpdateErrors0(dY,DY0sym,DYnewSym,Yshift);
    for (int k=0;k<Ni;k++) {
        dY[k]=MaxComp(YY[2*k+1]-YY[0],YY[0]-YY[2*k+2]);
    }
    UpdateErrors0(dY,DY0up,DYnewUp,dum);
    for (int k=0;k<Ni;k++) {
        dY[k]=MaxComp(YY[0]-YY[2*k+2],YY[2*k+1]-YY[0]);
    }
    UpdateErrors0(dY,DY0down,DYnewDown,dum);
}

void ePump::UpdateCorrCosine0(const d_vec &dY1, const d_vec &dY2, double &DY1, double &DY2,  double &Cos12, double &DY1new, double &DY2new, double &Cos12new, double &Y1shift, double &Y2shift)
                 // Give error vectors dY1 and dY2 as input.
{
    ...
    d_vec dY2T(Ni);
    for (int i=0;i<Ni;i++) {
        dY1T[i]=dY1[i]/TdynNorm[i];
        dY2T[i]=dY2[i]/TdynNorm[i];
    }
    BackSub(L,dY1,LinvdY1,Ni);
    BackSub(L,dY2,LinvdY2,Ni);
    BackSub(L,dY1T,LinvdY1T,Ni);
    BackSub(L,dY2T,LinvdY2T,Ni);
    Y1shift=dot(LinvdY1T,LinvA,Ni);
    Y2shift=dot(LinvdY2T,LinvA,Ni);
    DY1=sqrt(dot(dY1,dY1,Ni));
    DY2=sqrt(dot(dY2,dY2,Ni));
    DY1new=sqrt(dot(LinvdY1,LinvdY1,Ni));
    DY2new=sqrt(dot(LinvdY2,LinvdY2,Ni));
    Cos12=dot(dY1,dY2,Ni)/DY1/DY2;
    Cos12new=dot(LinvdY1,LinvdY2,Ni)/DY1new/DY2new;
}

void ePump::UpdateCorrCosine(const d_vec &YY1, const d_vec &YY2, double &DY1, double &DY2,  double &Cos12, double &DY1new, double &DY2new, double &Cos12new, double &Y1shift, double &Y2shift)
                 ...
                 // Uses symmetric error vectors.
{
    ...
    UpdateCorrCosine0(dY1,dY2,DY1,DY2,Cos12,DY1new,DY2new,Cos12new,Y1shift,Y2shift);
}
...
double ePump::Chi2Residuals(const d_vec &YY, int k)
{
    ...
    for (int a=0;a<Na[k];a++) {
        ...
        dchi2+=r[a]*r[a];
        cout<<setw(2)<<a+1<<scientific<<setprecision(4)<<setw(13)<<XE[k][a]<<setw(15)<<YY[a]+r[a]*s[k][a]<<setw(15)<<r[a]<<setw(15)<<rc2<<endl;
        ePout<<setw(2)<<a+1<<scientific<<setprecision(4)<<setw(13)<<XE[k][a]<<setw(15)<<YY[a]+r[a]*s[k][a]<<setw(15)<<r[a]<<setw(15)<<rc2<<endl;
    }
    cout<<endl;
    ePout<<endl;
    return dchi2;
...
//            Ydyn[j]=0.5*(UPD[2*j+1]-UPD[2*j+2])/TdynNorm[j];
        }
        BackSub(L,Ydyn,LinvYdyn,Ni);
        f0=UPD[0]+dot(LinvYdyn,LinvA,Ni);
...
// Central .dat file
        for (int i=0;i<nflavors;i++) {
            for (int j=0;j<Ni;j++) {
                ...
                Ydyn[j]=dPDF;
//            XXdyn[k][a][i]=ddPDF;
//                Ydyn[j]=0.5*(UPD[2*j+1][i]-UPD[2*j+2][i])/TdynNorm[j];
            }
            BackSub(L,Ydyn,LinvYdyn,Ni);
            f0[i]=UPD[0][i]+dot(LinvYdyn,LinvA,Ni);
...
//    double DY0sym,DY0up,DY0down,DYnewSym,DYnewUp,DYnewDown,Yshift;
    double Dsym,Dup,Ddown,DnewSym,DnewUp,DnewDown,Dshift;
    double DY1,DY2,Cos12,DY1new,DY2new,Cos12new,Y1shift,Y2shift;
    ...
    for (int k=0;k<Ndata;k++) {
        if (dataIncluded[k]) {
            NaTot+=Na[k];
            cout<<" Delta Chi2 for Data Set: "<<dataset[k]<<" with "<<Na[k]<<" data points"<<endl;
            cout<<" Old "<<dChi2[k]<<endl;
            cout<<" New "<<dChi2new[k]<<endl;
            ePout<<" Delta Chi2 for Data Set: "<<dataset[k]<<" with "<<Na[k]<<" data points"<<endl;
    ...
    cout<<endl;
    ePout<<endl;
    cout<<" Total Chi2 for new data sets with "<<NaTot<<" data points"<<endl;
    cout<<" Old "<<Chi2<<endl;
    cout<<" New "<<Chi2new<<endl;
    ePout<<" Total Chi2 for new data sets with "<<NaTot<<" data points"<<endl;
    ePout<<" Old "<<Chi2<<endl;
    ...
    cout<<" Total Chi2 for original global analysis plus new data sets "<<endl;
    cout<<" Old "<<Chi2<<endl;
    cout<<" New "<<Chi2new<<endl;
//    cout<<" New "<<Chi2newAlt<<endl;
    ePout<<" Total Chi2 for original global analysis plus new data sets "<<endl;
    ePout<<" Old "<<Chi2<<endl;
    ePout<<" New "<<Chi2new<<endl;
//    ePout<<" New "<<Chi2newAlt<<endl;
    ...
    for (int k=0;k<Ndata;k++) {
        ...
        for (int a=0;a<Na[k];a++) {
            UpdateErrors(Xmat[k][a],Dsym,Dup,Ddown,DnewSym,DnewUp,DnewDown,Dshift);
            cout<<"Old"<<setw(4)<<a+1<<"  "<<setw(11)<<setprecision(4)<<scientific<<X0[k][a]<<"  "<<Dsym<<"       -"<<Ddown<<"  +"<<Dup<<endl;
            cout<<"New"<<setw(4)<<a+1<<"  "<<setw(11)<<setprecision(4)<<scientific<<X0[k][a]+Dshift<<"  "<<DnewSym<<"       -"<<DnewDown<<"  +"<<DnewUp<<endl<<endl;
            ePout<<"Old"<<setw(4)<<a+1<<"  "<<setw(11)<<setprecision(4)<<scientific<<X0[k][a]<<"  "<<Dsym<<"       -"<<Ddown<<"  +"<<Dup<<endl;
            ePout<<"New"<<setw(4)<<a+1<<"  "<<setw(11)<<setprecision(4)<<scientific<<X0[k][a]+Dshift<<"  "<<DnewSym<<"       -"<<DnewDown<<"  +"<<DnewUp<<endl<<endl;
        }
        cout<<endl;
        ePout<<endl;
    }
            ...
            for (int j=k;j<Ndata;j++) {
                ...
                for (int a=0;a<Na[k];a++) {
                    ...
                    for (int b=0;b<Na[j];b++) {
                        ...
                        UpdateCorrCosine(Xmat[j][b], Xmat[k][a], DY1, DY2,Cos12, DY1new, DY2new,Cos12new,Y1shift,Y2shift);
                        cout<<Cos12<<"  ";
                        ePout<<Cos12<<"  ";
                    }
                    cout<<endl;
